 ...
 */
package org.openmrs.module.gbv.wrapper;
...
import org.openmrs.module.gbv.AbstractObjectWrapper;
...
public class Enrollment extends AbstractObjectWrapper<PatientProgram> {
    ...
    public Obs firstObs(Concept concept) {
        ...
        for (Obs obs : obss) {
            if (obs.getObsDatetime().compareTo(target.getDateEnrolled()) >= 0 && (target.getDateCompleted() == null
                    || obs.getObsDatetime().compareTo(target.getDateCompleted()) < 0)) {
                return obs;
            }
        }
        return null;
    ...
    public Encounter lastEncounter(EncounterType type) {
        List<Encounter> encounters = Context.getEncounterService().getEncounters(target.getPatient(), null,
                target.getDateEnrolled(), target.getDateCompleted(), null, Collections.singleton(type), null, null,
                null, false);
